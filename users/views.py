...
class ProfileUpdateView(CreateModelMixin, RetrieveModelMixin,UpdateModelMixin, DestroyModelMixin, GenericViewSet):
...
def password_change_view(request):
    if request.method == 'POST':
        ...
        if user.check_password(old_password):
            ...
            return Response({'message': 'Your old password was wrong'}, status=status.HTTP_400_BAD_REQUEST)
    else:
        return Response(
            data={'status': 'fail', 'description': 'Only POST method allowed'},
...
def password_reset_view(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return Response({'message': 'fail', 'description': str(e)}, status=status.HTTP_400_BAD_REQUEST)
